...
function excution()
{
    var eliminateDefinateArr;
//出口
//出口口岸
/*    var exportPort = main(CustomCustom, InsGlobalDistrict, true, 1.6);
    //输往国家
    var importingCountry = main(CustomCountry, InsCountryZone, true, 1.6);
    //到达口岸
    var arrivaloPort = main(CustomPort, InsGlobalPort, true, 1.6);*/

    //进口
    //贸易国、启运国
    var tradeCountryAndStartingTransportCountry = main(CustomCountry, InsCountryZone, true, 1.1);
    //启运口岸
    var startingTransportPort = main(CustomPort, InsGlobalPort, true, 1.2);
    //入境口岸
    var entryPort = main(CustomCustom, InsPort, true, 1.2);
//原产国
    eliminateDefinateArr = [["巴林", "巴林"],['朝鲜', '朝鲜'], ['黑山', '黑山'],['蒙古', '蒙古'], ['古巴','古巴']];//用于剔除融合数组元素
    var originCountry = main(CustomCountry, InsDistrict, true, 1, eliminateDefinateArr);

//进出口共有
//币制
    var currencyCategories = main(CustomCurr, InsCurrency, true, 1.6);
}
//(bool)special:控制是否一对一，true为一对一，false为一对多；(int)defaultPercent>=1:控制匹配程度，越小越好，不能小于1，1为完全匹配
function main(arr1, arr2, special, defaultPercent,eliminateDefinateArr)
{
    var reflectionList = [];
    var same = [];
    var diff_arr1 = [];
    ...
    var i = 0;
    var j = 0;
    var selfDefinateArr = [["台澎金马关税区", "中国台湾"]];//设置特定对应关系，用于生成原始reflection（-。-算法没法识别）
    //1.匹配国家无歧义相同项生成映射数组
...
//        showDiff(diff_arr1, diff_arr2);//对比数组
    ...
    if(eliminateDefinateArr) {
        dimension2_removeElemOfArr(fixed, eliminateDefinateArr);
    }
...
//剔除二维数组某元素
function dimension2_removeElemOfArr(arr1, arr2) {
